...
#include <___spi_lcd/lcd_display.h>
...
sl_status_t lcd_tx_1line(const struct sl_memlcd_t *device, const void *data, unsigned int row_start, unsigned int row_count) {
    uint16_t cmd;
    uint16_t dummy;

    for (unsigned int row = row_start; row < row_start + row_count; row++) {
      GPIO_PinOutSet(SL_MEMLCD_SPI_CS_PORT, SL_MEMLCD_SPI_CS_PIN);
      ...
      sli_memlcd_spi_tx(&spi_handle, &dummy, 2);
      sli_memlcd_spi_wait(&spi_handle);

      sl_udelay_wait(device->hold_us);               // delay

      GPIO_PinOutClear(SL_MEMLCD_SPI_CS_PORT, SL_MEMLCD_SPI_CS_PIN);
      data = (const uint8_t*)data + 16;
    }

  return SL_STATUS_OK;
}
sl_status_t lcd_tx_multi(const struct sl_memlcd_t *device, const void *data, unsigned int row_start, unsigned int row_count) {
    uint16_t cmd;
    uint16_t dummy;
...
sl_status_t lcd_tx(const struct sl_memlcd_t *device, const void *data, unsigned int row_start, unsigned int row_count) {
  lcd_tx_1line(device, data, row_start, row_count);

//  lcd_tx_multi(device, data, row_start, row_count);
